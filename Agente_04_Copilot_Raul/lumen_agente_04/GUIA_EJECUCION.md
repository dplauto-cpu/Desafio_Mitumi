...
- Conexión a internet (para las preguntas que use el LLM de Groq; el resto del agente funciona sin red).
...
Instala `openai` (cliente para hablar con Groq, API compatible con OpenAI) y `flask`/`flask-cors`
(solo necesarios para `servidor.py`, la API HTTP que usa el frontend React — ver punto 4.3).

## 3. Configurar el `.env`

El archivo `.env` **ya viene incluido con la API key de Groq real**, lista para usar. No hace falta tocar nada para probar el agente tal cual.
...
el evento 12 y luego dices "¿y su presupuesto?" o "ese evento", Lumen recuerda a qué evento te
refieres sin que lo repitas — avisando siempre cuándo está usando esa memoria. Escribe `nuevo`
para olvidar el contexto sin reiniciar el proceso, y `salir` para terminar.
...
Este es el comportamiento **original** de `main.py`: lee la pregunta fija de
`inputs/payload_demo.json` ("¿qué ponentes no han subido el billete de vuelta?", sobre el evento
12), llama a `ejecutar_agente(payload)` una vez, imprime el payload y la respuesta completa en
JSON, y la guarda en `outputs/respuestas_json/salida_demo.json`. Útil para verificar rápido que
nada se ha roto, sin depender de escribir preguntas a mano.

Salida esperada (resumen):

```
"resumen": "El evento 12 tiene 2 ponente(s) sin billete de vuelta: Ana Ruiz, Marc Sole."
```
...
preguntas = [
    "¿cuál es el presupuesto total aprobado y cuánto es de catering?",
    "¿en qué sala y espacio se celebra el evento, y cuál es el aforo?",
    "¿qué ponentes no han subido el billete de ida?",
]

for pregunta in preguntas:
    payload = {**payload_base, "datos": {"pregunta": pregunta}}
