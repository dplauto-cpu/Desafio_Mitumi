...
> **Esta es la copia `Demo_sin_IA/`: motor `reglas` (gratis, sin llamadas a
> ningún proveedor de IA) por defecto** — es el contenido que estaba
> preparado para subir a `DESAFIO_MITUMI/agente_operis_autocompletar_Ainara_Dv/`,
> movido aquí de vuelta a la carpeta de trabajo local. Las copias hermanas
> son `Agente_Operis/agente_operis/` (misma configuración, copia de trabajo
> principal) y `Agente_Operis/agente_operis_llm/` (motor `llm`/Groq por
> defecto, para probar con IA real). El repo (`DESAFIO_MITUMI/`) por ahora
> solo tiene un README de marcador — ver ahí para el estado actual.
